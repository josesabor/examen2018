...
INSERT INTO usuarios (nombre, PASSWORD)
  VALUES ('pepe', crypt('pepe', gen_salt('bf', 11))), ('juan', crypt('juan', gen_salt('bf', 11)));
...
INSERT INTO aeropuertos (codigo, denominacion)
  VALUES ('XRY', 'Jerez'), ('SVQ', 'Sevilla'), ('STD', 'Londres Stansted');
...
INSERT INTO companias (denominacion)
  VALUES ('Iberia'), ('Ryanair');

DROP TABLE IF EXISTS vuelos CASCADE;

CREATE TABLE vuelos (
  id bigserial PRIMARY KEY,
  codigo varchar(6) UNIQUE CONSTRAINT ck_codigo_valido CHECK (codigo ~ '^[A-Z]{2}\d{4}$'),
  origen_id bigint NOT NULL REFERENCES aeropuertos (id) ON DELETE NO ACTION ON UPDATE CASCADE,
  destino_id bigint NOT NULL REFERENCES aeropuertos (id) ON DELETE NO ACTION ON UPDATE CASCADE,
  compania_id bigint NOT NULL REFERENCES companias (id) ON DELETE NO ACTION ON UPDATE CASCADE,
...
CREATE TABLE reservas (
  ...
  vuelo_id bigint NOT NULL REFERENCES vuelos (id) ON DELETE NO ACTION ON UPDATE CASCADE,
